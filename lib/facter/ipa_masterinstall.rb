...
Facter.add("ipa_masterinstall") do
  setcode do
    ...
    install = system("/usr/bin/pythond -c 'import os,sys,ipapython.sysrestore; sys.exit(0 if ipapython.sysrestore.FileStore(\"/var/lib/ipa/sysrestore\").has_files() else 1)'>/dev/null 2>&1")
