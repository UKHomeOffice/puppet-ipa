...
Facter.add("ipa_clientinstall") do
  setcode do
    ...
    install = system("/usr/bin/python -c 'import os,sys,ipapython.sysrestore; sys.exit(0 if ipapython.sysrestore.FileStore(\"/var/lib/ipa-client/sysrestore\").has_files() else 1)'>/dev/null 2>&1")
